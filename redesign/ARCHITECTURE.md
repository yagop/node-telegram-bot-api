...
4. **The library performs no serialization.** Structured fields (`reply_markup`, entities, …) are typed as **branded `Json<T>` strings**: their values arrive already wire-ready. Users produce them with builders (`new InlineKeyboard()…build()`) or the generic `json(value)`. The request pipeline never stringifies anything — which is exactly what removes the need for a second client layer. No `_fix*`, no serialize step.
...
- Every `Api` method takes a **single argument**.
- The request pipeline performs **no serialization**: values arrive wire-ready.
...
class Api {
  ...
  // GENERATED — one concrete method per Bot API method, each single-argument:
  getMe(): Promise<User>;
  getUpdates(params?: GetUpdatesParams): Promise<Update[]>;
  sendMessage(params: SendMessageParams): Promise<Message>;
  sendPhoto(params: SendPhotoParams): Promise<Message>;
  // …one per documented method; each body is just `return this.request("name", params)`
}
```

There is **no `RawApi`/`Api` split**. The only thing a second layer would have done was serialize arguments before delegating; since the library does no serialization (§6.2), there is nothing for it to do, so one class suffices. `Bot` holds an `Api`; `ctx.api` and `bot.api` expose it.

Why a class over a `Proxy`: real methods give correct stack traces, are greppable and debuggable, tree-shake per method, and need no `as` casts to type. Adding a Bot API method means regenerating `Api` — still no hand-written body. Positional sugar (`reply(text)`) lives on `Context`, keeping the client a clean mirror of the wire API. (gotgbot in Go takes exactly this generated-concrete-method approach — see [`RESEARCH-go-rust-clients.md`](./RESEARCH-go-rust-clients.md).)

### 6.2 Serialization — branded `Json<T>` strings; the library serializes nothing

Telegram's wire wants structured fields (`reply_markup`, entities, `reply_parameters`, …) as **JSON-serialized strings** on the form body. v2 takes that literally: those fields are **typed as strings**, and the value is serialized **at the call site**, before it ever reaches the client. The request pipeline only ever moves strings.
...
**Nested files (e.g. `sendMediaGroup`)** are the one case where a JSON array must reference files by `attach://<name>` while the bytes travel as separate parts. This is handled by a builder, so the "library serializes nothing" rule still holds:
...
`Transport` is the only module touching `fetch`; it is injectable (`opts.fetch`) so unit tests pass a fake instead of monkeypatching `globalThis.fetch`. It merges timeout + caller signal with `AbortSignal.any`, unwraps the envelope, and retries `429` honoring `retry_after`. Errors preserve `cause` and expose structured fields: `TelegramApiError` carries `errorCode`/`description`/`parameters` (and a `retryAfter` getter) so callers branch on values, not message substrings.
...
4. **Structured args are branded `Json<T>` strings.** `reply_markup`/entities/etc. take the output of a builder (`new InlineKeyboard()…build()`) or `json(value)`. Passing a plain object or a bare string is a type error. The `_fix*` pipeline and any library-side serialization are gone.
...
**Decision: D.** Rejected B (Proxy) for poor stack traces and casts; rejected C's second layer as unnecessary once serialization left the pipeline. Pros: smallest surface, plain debuggable OO, methods can't drift from types. Cons: generated output larger than a Proxy — acceptable, it's mechanical. (Direct precedent: gotgbot generates concrete methods in Go; see the research note.)

### ADR-002 — Structured args are branded `Json<T>` strings; the library serializes nothing
...
**Consequences.** `if ('message' in u)` narrows; `Context` getters are sound. Trade-off: generator change + a one-time consumer migration.
...
**Decision: B.** One field model for every request: every field is a string; multipart only adds file parts. File uploads are common in Telegram, so the JSON-body optimization would rarely apply, and a single field model is simpler to reason about and pairs naturally with ADR-002 (values arrive as `Json<T>` strings).
**Consequences.** No body-type branching beyond "is a file present." The trade-off — losing JSON-body's automatic nesting — is exactly what ADR-002 already handles by serializing at the call site.
...
- **Serverless response timing.** Workers should ACK Telegram fast and finish work in `ctx.waitUntil`. v2 should expose a hook so handlers can return early; the default awaits fully.
- **Concurrency model for polling.** Sequential by default; provide an opt-in concurrency/queue middleware rather than baking it in.

*(Resolved: keep the name `node-telegram-bot-api`, one package with subpath exports — ADR-009; a single generated `Api` class, no Proxy, no Raw/Api split — ADR-001; structured args as branded `Json<T>` strings built at the call site, library serializes nothing — ADR-002; uniform form encoding, no JSON body — ADR-010; `InputFile`/multipart and nested uploads via `mediaGroup()` — ADR-011.)*
