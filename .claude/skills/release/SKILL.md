...
> A prerelease like `-alpha.1` / `-rc.0` publishes under a non-`latest` tag (see section 8).
...
> `doc/api.md` is the generated API reference and is tracked in git. If the release changes
> public declarations or their documentation, run `npm run generate:docs` and include the
> updated reference. See the [generate-docs skill](../generate-docs/SKILL.md).
