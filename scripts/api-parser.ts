...
function transformParamType(ts: string): string {
  ...
  // Structured (array or object reference): wire-ready JSON. `Json<T>` accepts either
  // a JsonString<T> (json()/a file-free builder) or a file-carrying FilePart<T>, so a
  // field that can upload needs no special type - hence no file-bearing allow-list.
  return `Json<${t}>`;
}
