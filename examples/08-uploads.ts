 ...
 *   - `mediaGroup()` — build a `sendMediaGroup` payload; uploaded files are wired in
 *     via `attach://` references for you.
...
if (chatId !== 0) {
  ...
  // 4) An album. `mediaGroup().build()` mints attach:// refs for any InputFile.
