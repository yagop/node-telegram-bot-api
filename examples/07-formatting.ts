 ...
 *
 * `fmt()` is a fluent builder that accumulates text and tracks UTF-16 offsets for
 * you, so you never hand-count entity positions or escape Markdown/HTML. Its
 * `.build()` returns `{ text, entities }` — both ready to drop into `sendMessage`.
 * `EntityType` is a typo-proof enum of every documented entity kind.
 *
 * Run: BOT_TOKEN=123:abc CHAT_ID=12345 bun examples/07-formatting.ts
 */
import { Api, fmt, EntityType, json } from "node-telegram-bot-api";
